...
| :--- | :--- | :--- | :--- |
| **Supply Chain** | Compromised dependencies or malicious base images | Digest pinning, automated SAST/SBOM, Trivy scanning | `trivy-action`, Hadolint, pinned action SHAs |
...
The following policies are enforced across the cluster:
...
3. **`require-read-only-rootfs`:** Enforces `readOnlyRootFilesystem: true`.
